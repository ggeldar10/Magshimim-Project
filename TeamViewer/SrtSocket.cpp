...
void SrtSocket::listenAndAccept()
{
	...
	toAddr.sin_addr.s_addr = ntohl(this->_commInfo._dstIP);
...
void SrtSocket::connectToServer(sockaddr_in* addrs) //todo add the waitForValidPacket
{
	...
	this->_commInfo._dstIP = htonl(addrs->sin_addr.s_addr);
...
void SrtSocket::sendImageStream()
{
	...
	while (runLoop)
	{
		...
		int amountOfPackets = length / this->_commInfo._otherComputerMTU;
		if (length % this->_commInfo._otherComputerMTU != 0)
		{
			amountOfPackets++;
		}
		amountOfPackets -= 1;
